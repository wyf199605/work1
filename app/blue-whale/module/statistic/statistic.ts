...
interface StatisticBasicPara {
    ...
    colDataGet(data : string) : obj[];//根据列名获取列所有数据
    paraGet() : obj//表格参数的获取
    getVisibleCol() : obj;
}

// declare const tablePageObj:PcTablePage;
export = class StatisticBasic extends StatisticBase{
    ...
    * */
    private colStatistic(cols: string[], stats: { title: string, method: (nums: number[]) => number }[], groupCol: string) {
        let self = this,
            confCols = cols.map((col) => self.para.cols[self.colName2index(col)]), // 参数 cols
