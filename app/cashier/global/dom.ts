...
let event = (function () {
    ...
    // 委托元素对应的回调
    let eventHash = elemDataInnerKey.event,
        noDelegateSelector = '';
            ...
            pan: {
                ...
                on (el:Node, selector:string) {
                    ...
                    eventOn(el, EVENT_START, selector, panHandlers.start = (ev:MouseEvent|TouchEvent) => {
                        ...
                        eventOn(document, EVENT_MOVE, panHandlers.move = (ev) => {
                            this.handlers.move.call(el, ev);
                        });

                        // eventOn(el, EVENT_END, this.handlers.end);
                        eventOn(document, EVENT_END, panHandlers.end = (ev) => {
    ...
    let eventOn: IEventOnPara = function (el, types, selector, cb?) {
        ...
        eventArrayGet(types).forEach(function (type) {
            ...
            if (first) {
                ...
                try{
                    el.addEventListener(type, eventCache.dispatcher,
                        passiveTypes.indexOf(type) > -1 ? {passive: true} : false);
                }catch (e){
    ...
    setPosition(el: HTMLElement, relEl: HTMLElement, useRelWidth = true) {
        if(el instanceof HTMLElement && relEl instanceof HTMLElement){
            ...
            let relRect = relEl.getBoundingClientRect(),
                ...
                top = relRect.bottom + 3,
                left = relRect.left - 3;

            top = top + elHeight < bodyHeight ? top : relRect.top - elHeight - 3;
...
function h(tag, props?: obj, ...children ) {
    ...
    if(Array.isArray(children)) {
        ...
        children.forEach(child => {
            let node = child instanceof Node ? child : document.createTextNode(child);
