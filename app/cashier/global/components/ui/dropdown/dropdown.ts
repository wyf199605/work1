...
export class DropDown {
    ...
    constructor(private para: DropDownPara) {
        this.ulDom = <HTMLUListElement>d.create('<ul class="dropdown-menu"></ul>');
        if (para.className) {
    ...
    public showList(relEl?: HTMLElement) {
        ...
        if (ajax && ajax.fun) {
            ...
            ajax.fun(ajax.url, selectedData ? selectedData.value.toString() : '', d => {
                if (Array.isArray(d)) {
                    this.initListData(d);
                }
                spinner.hide();
    ...
    private render(data: ListItem[], isRefresh = true) {
        ...
        data.forEach((da, i) => {
            if (!tools.isEmpty(da)) {
                items.push(
                    d.create(
                        `<li class="drop-item" title="${tools.str.removeHtmlTags(da.text)}" data-index="${plus + i}"><span class="drop-span">${da.text}</span></li>`));
            }
        });
