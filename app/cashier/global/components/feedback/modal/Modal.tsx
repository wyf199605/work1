...
export class Modal extends Component {
    ...
    protected wrapperInit(para: IModal): HTMLElement {
        let wrapper = <div tabindex={parseInt(tools.getGuid(''))} className="modal-wrapper"></div>;
        return wrapper;
    }
    ...
    private set header(header: IModalHeaderPara | string) {
        ...
        this._modalHeader = new ModalHeader(tools.obj.merge({
    ...
    get bodyWrapper(){
        if(!this._bodyWrapper){
            this._bodyWrapper = <div className={`modal-body${tools.isMb ? ' modal-body-mobile' : ''}`}></div>;
    ...
    static toast(msg: any) {
        ...
        let m = new Modal({
            isBackground: false,
            body: <div style="padding: 4px 15px;">{tools.str.htmlEncode(msg)}</div>,
