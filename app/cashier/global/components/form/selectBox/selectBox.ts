...
interface SelectBoxPara extends ComPara{
    select: {
        multi: boolean,
...
export class SelectBox extends BasicCom{
    onSet: (val) => void;
    ...
    constructor(private para: SelectBoxPara) {
    ...
    private addByList() {
        Array.isArray(this.para.data) && this.para.data.forEach((obj, i) => {
            let selectBox = CheckBox.initCom(this.guidName, obj.text ? obj.text : '', this.type),
    ...
    addByItem(el: HTMLElement) {
        let selectBox = CheckBox.initCom(this.guidName);
