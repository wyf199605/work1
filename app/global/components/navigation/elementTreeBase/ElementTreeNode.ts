...
import {IBasicTreePara, TreeNodeBase} from "../../../dataStruct/tree/TreeNodeBase";
...
export type IElementTreeAjax<T extends ElementTreeNode, K extends IElementTreeNodePara> =
    IElementTreeAjaxHandler<T, K>
    | false;
...
export class ElementTreeNode extends TreeNodeBase implements IComponentPara {
    ...
    protected get expandIconEl() {
        if (!this._expandIconEl) {
            let pre = tools.isNotEmpty(this.expandIconPre) ? this.expandIconPre : 'iconfont';
    ...
    childrenAdd(para: this | IElementTreeNodePara | (IElementTreeNodePara | this)[]): this[] {
    ...
    protected expandIcon() {
        let iconArr = tools.isNotEmpty(this.expandIconArr) ? this.expandIconArr : ['icon-zhankaishousuo-shousuo', 'icon-zhankaishousuo-zhankai'];
        return iconArr;
    }
    ...
    set expand(expand: boolean) {
        ...
        setTimeout(() => {
            ...
            let isAjax = this.ajax && !this.disabled && expand && tools.isEmpty(this.children);
    ...
    set isAccordion(isAccordion: boolean) {
        ...
        if (this._isAccordion) {
            let children = this.children, firstExpand = false;
    ...
    protected doAjax() {
        ...
        return new Promise((resolve, reject) => {
            if (this.ajax) {
                this.ajax(this).then((para: IElementTreeNodePara[]) => {
                    // debugger;
                    callback(para);
                    resolve();
                }).catch(() => {
