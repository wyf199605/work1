...
(function() {
	...
	function getFileName() {
        ...
        let name = window.location.pathname.match(reg);
        return name? name[1]: '';
    };
    function closeMenu() {
    ...
    function selectMenu() {
    	...
    	let currentLink = leftmenu.querySelectorAll('.panel-collapse a');
	    for (let i = 0; i < currentLink.length; i++) {
	    	let linkName = currentLink[i].getAttribute('href').replace('.html', '');
			if(linkName == currentFileName) {
				let current = closest.apply(currentLink[i], ['.panel-collapse']);
				if(current) {
    ...
    function bindEvent() {
    	[].forEach.call(leftmenu.querySelectorAll('.panel-title'), function(a){　　　　　　　
			a.addEventListener('click', (event)=> {
				let panel = closest.apply(event.target, ['.panel']);
				closeMenu();
